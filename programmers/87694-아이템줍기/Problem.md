...
- 서로 다른 두 직사각형의 x축 좌표 또는 y축 좌표가 같은 경우는 없습니다. 즉, 서로 다른 두 직사각형이 꼭짓점에서 만나거나, 변이 겹치는 경우 등은 없습니다.
- 지형이 2개 이상으로 분리된 경우는 없습니다.
...
- `characterX`, `characterY`는 1 이상 50 이하인 자연수입니다. 지형을 나타내는 다각형 테두리 위의 한 점이 주어집니다.
- `itemX`, `itemY`는 1 이상 50 이하인 자연수입니다. 지형을 나타내는 다각형 테두리 위의 한 점이 주어집니다.
...
**입출력 예 #1**
캐릭터 위치는 (1, 3)이며, 아이템 위치는 (7, 8)입니다. 다각형 테두리(굵은 선)를 따라 이동하는 경로가 가장 짧습니다.

**입출력 예 #2**
캐릭터 위치는 (9, 7)이며, 아이템 위치는 (6, 1)입니다.
