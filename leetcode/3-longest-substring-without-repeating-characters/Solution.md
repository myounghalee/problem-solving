...
- [Minimum Window Substring (LeetCode 76)](../76-minimum-window-substring/Solution.md) — 같은 가변 크기 슬라이딩 윈도우지만 방향이 반대다. 이 문제는 "조건이 깨지면 줄이는" 반면 76번은 "조건을 만족하면 줄여보는" 구조라, 최솟값/최댓값을 갱신하는 위치도 서로 반대(while을 나온 뒤 vs while 안에서)다.
